...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "flex w-full items-center justify-between gap-2 rounded-lg border px-3 py-2 text-sm whitespace-nowrap transition-all outline-none",
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          "relative z-50 max-h-(--radix-select-content-available-height) min-w-[8rem] origin-(--radix-select-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-lg border shadow-[var(--shadow-lg)]",
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      ...
      className={cn("px-2 py-1.5 text-xs text-[var(--text-tertiary)] font-semibold", className)}
      {...props}
    />
  )
}
    ...
    >
      <span className="absolute right-2 flex size-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <CheckIcon className="size-4 text-[var(--accent-cyan)]" />
        </SelectPrimitive.ItemIndicator>
      </span>
...
function SelectSeparator({
  ...
  return (
    <SelectPrimitive.Separator
      ...
      className={cn("pointer-events-none -mx-1 my-1 h-px bg-[var(--border-default)]", className)}
      {...props}
    />
  )
}
